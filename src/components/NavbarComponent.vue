...
<template>
	<Disclosure as="nav" class="bg-gray-800 p-2" v-slot="{ open }">
			...
			<div class="relative flex h-16 items-center justify-between">
				<div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
					...
					>
						...
						<FontAwesomeIcon :icon="faBars" v-if="!open" class="block h-6 w-6" aria-hidden="true" />
						<FontAwesomeIcon :icon="faXmark" v-else class="block h-6 w-6" aria-hidden="true" />
					</DisclosureButton>
				...
				<div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
					<div class="hidden sm:flex sm:items-center">
						...
						<div class="sm:ml-6 flex space-x-4">
							<RouterLink
		...
		<DisclosurePanel class="sm:hidden">
			<div class="space-y-1 px-2 pb-3 pt-2">
				<RouterLink
					...
					:key="item.name"
					:to="item.href"
					class="text-gray-300 hover:bg-gray-700 hover:text-white block rounded-md px-3 w-full py-2 text-base font-medium"
					active-class="bg-gray-900 text-white"
				>
					{{ item.name }}
				</RouterLink>
